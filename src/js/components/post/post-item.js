...
export class PostArticle extends HTMLElement {
  constructor() {
    super();
    this.postData = {};
  }
  ...
  /**
   * Creates the post element based on post object
   * @param {{ id, title, body, tags, media, author, _count, created, updated }} post - Post data
   * @returns Post HTMLElement
   */
  createPost(post) {
    const article = htmlUtilities.createHTML("post-article");
    article.postData = post;
    return article;
  }

  /**
   * Creates and appends posts to parent
   * @param {Array} posts - Array of posts
   * @param {HTMLElement} parent - Element to append posts to
   */
  renderPosts(posts, parent) {
    const elements = [];
    for (const post of posts) {
      const li = htmlUtilities.createHTML("li");
      li.append(this.createPost(post));
      elements.push(li);
    }
  ...
  createHeader(author, created, updated) {
    ...
    const img = htmlUtilities.createHTML(
      ...
      null,
      {
        src: author?.avatar ?? "/assets/images/avatar-placeholder.jpg",
        alt: author.name,
      },
    );
